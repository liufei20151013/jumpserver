...
class OAuth2AuthCallbackView(View, FlashMessageMixin):
    ...
    def get(self, request, params):
        ...
        # 将参数拆分为字典（支持多值参数）
        param_dict = parse_qs(decoded_params)
        ...
        ticket_list = param_dict.get('ticket', [])
        ticket = ticket_list[0] if ticket_list else ''
        logger.debug('*********ticket:' + ticket)

        logger.debug('**********************' + json.dumps(callback_params))
        if len(ticket) > 0:
        # if 'ticket' in callback_params:
            logger.debug('**********************1')
            logger.debug(log_prompt.format('Process authenticate'))
            # user = authenticate(code=callback_params['ticket'], request=request)
            user = authenticate(code=ticket, request=request)
