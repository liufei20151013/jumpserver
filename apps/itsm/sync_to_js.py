...
def sync_local_mfa_to_other_js(username, otp_secret_key):
    ...
    if not enabled:
        logger.info('当前本地用户 MFA 重置后，同步到其它 JumpServer 环境的功能未开启, 不需要处理')
        return
    ...
    try:
        ...
        logger.info("JS url size: {}".format(len(jms_url_arr)))
        logger.info("JS Access Key size: {}".format(len(key_id_arr)))
        logger.info("JS Secret Key size: {}".format(len(secret_id_arr)))
    except Exception as e:
        traceback.logger.info_exc()
        logger.error('账号参数配置不规范：{}'.format(e))
        return

    for index, jms_url in enumerate(jms_url_arr):
        try:
            logger.info("Handling data, index: {}".format(index + 1))
            logger.info("Handling data, jms_url: {}".format(jms_url))
            KeyID = key_id_arr[index]
            SecretID = secret_id_arr[index]
            auth = get_auth(KeyID, SecretID)

            # todo 根据 username 查询用户
            ...
            traceback.logger.info_exc()
            logger.error('同步 MFA 的接口调用失败：{}'.format(e))
            continue
...
def update_user_mfa(jms_url, auth, user_id, username, otp_secret_key):
    ...
    if response.status_code == 200:
        logger.info("Success to update user[{}]'s MFA.".format(username, response.reason))
    else:
        logger.error("Failed to update user[{}]'s MFA, error:{}.".format(username, response.reason))
